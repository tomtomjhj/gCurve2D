...
public:
  ...
  void run() {
    ...
    for (int i = 0; i < _runNum; ++i) {
      ...
      std::cout << "TIME: " << stats.totalTime << "(" << stats.initTime << ", "
                << stats.splitTime << ", " << stats.flipTime << ", "
                << stats.relocateTime << ", " << stats.sortTime << ", "
                << stats.constraintTime << ", " << stats.outTime << ")"
                << std::endl;
    ...
    Visualizer::instance()->addFrame(_input.pointVec, _input.constraintVec,
                                     _output.triVec);
#endif
  }
};
...
void summarize(const App &app) {
  ...
  std::cout << "Sort           " << (app._input.noSort ? "no" : "yes")
            << std::endl;
  std::cout << "Reorder        " << (app._input.noReorder ? "no" : "yes")
            << std::endl;
  std::cout << "Insert mode    " << (app._input.insAll ? "InsAll" : "InsFlip")
            << std::endl;
  std::cout << std::endl;
  std::cout << std::fixed << std::right << std::setprecision(2);
  std::cout << "TotalTime (ms) " << std::setw(10) << stats.totalTime
            << std::endl;
  std::cout << "InitTime       " << std::setw(10) << stats.initTime
            << std::endl;
  std::cout << "SplitTime      " << std::setw(10) << stats.splitTime
            << std::endl;
  std::cout << "FlipTime       " << std::setw(10) << stats.flipTime
            << std::endl;
  std::cout << "RelocateTime   " << std::setw(10) << stats.relocateTime
            << std::endl;
  std::cout << "SortTime       " << std::setw(10) << stats.sortTime
            << std::endl;
  std::cout << "ConstraintTime " << std::setw(10) << stats.constraintTime
            << std::endl;
